...
export class PensionadosService {
  ...
  // Recalcular y persistir el totalMonto del período
  async actualizarTotalPeriodo(periodoId: number): Promise<PeriodoFacturacion> {
    const total = await this.calcularTotalPeriodo(periodoId);
    await this.periodoRepo.update(periodoId, { totalMonto: total });
    return this.findOnePeriodo(periodoId);
  }
}
